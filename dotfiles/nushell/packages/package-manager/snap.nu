...
def --env "pm install snap" [$package: string] {
    if ((pm status snap) == false) {
        return false
    }
...
def --env "pm update snap" [$package: string] {
    if ((pm status snap) == false) {
        return false
    }
...
def --env "pm uninstall snap" [$package: string] {
    if ((pm status snap) == false) {
        return false
    }
    ^snap remove $package
