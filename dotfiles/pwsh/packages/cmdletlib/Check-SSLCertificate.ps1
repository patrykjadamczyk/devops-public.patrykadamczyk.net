...
function Global:Check-SSLCertificate (
    [Parameter(Mandatory=$true, Position=0)]
    [String] $Address
) {
    if ($DebugPreference -ne "SilentlyContinue") {
        Write-Host -ForegroundColor DarkRed "Warning! Check-SSLCertificate is added only once as type. Changes of it after reloading of profile file or executing this cmdlet again will not apply to same session!";
    }
